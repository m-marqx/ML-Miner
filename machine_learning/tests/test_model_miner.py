...
class ModelMinerTest(unittest.TestCase):
    def setUp(self):
        np.random.seed(120)
        self.dataframe = pd.read_parquet(r"data\cryptodata\btc.parquet").loc[
            :"2023"
        ]
    ...
    def test_init(self):
        self.assertEqual(
            self.model_miner.ohlc, ["open", "high", "low", "close"]
        )
        ...
        pd.testing.assert_frame_equal(
            self.model_miner.dataframe, self.dataframe
        )
    ...
    def test_generate_feat_parameters(self):
        ...
        pd.testing.assert_series_equal(
            pd.Series(feat_parameters), pd.Series(expected_feat_parameters)
        )
    ...
    def test_search_model(self):
        expected_results = {
            "feat_parameters": [
                {
                    "random_features": ("Stoch_opt", "CCI", "MACD_opt"),
        ...
        with warnings.catch_warnings():
            ...
            np.random.seed(0)
            results = ModelMiner(self.dataframe, self.target).search_model(
                1030
            )

        pd.testing.assert_series_equal(
            pd.Series(results).drop("total_time"), pd.Series(expected_results)
        )
    ...
    def test_warning_search_model(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            assert (
                "signal isn't compatible with normalization and will be set to 'histogram'."
                in str(w[-1].message)
            )
    ...
    def test_create_model(self):
        ...
        results = self.model_miner.create_and_calculate_metrics(
            feat_params, hyperaparams, 1030
        )
        ...
        pd.testing.assert_series_equal(
            pd.Series(results).drop("total_time"), expected_results
        )
